...
// Upper bound for the generated TLS ClientHello. tempBuffer holds 64 KB, so this
// is a conservative ceiling that guarantees we never write past the buffer.
static constexpr uint32_t CLIENT_HELLO_LIMIT = 2048;

// Cryptographically secure replacements for rand(): the fake ClientHello must be
// unpredictable so that DPI cannot fingerprint it by a weak PRNG pattern.
static uint32_t tls_random_u32() {
    uint32_t value = 0;
    RAND_bytes(reinterpret_cast<unsigned char *>(&value), sizeof(value));
    return value;
}

static size_t tls_random_index(size_t bound) {
    if (bound <= 1) {
        return 0;
    }
    return static_cast<size_t>(tls_random_u32() % static_cast<uint32_t>(bound));
}
...
static bool generate_public_key(unsigned char *key) {
    BIGNUM *mod = NULL;
    ...
    if (mod == NULL || pow == NULL || big_num_context == NULL) {
        BN_clear_free(mod);
        BN_clear_free(pow);
        if (big_num_context != NULL) BN_CTX_free(big_num_context);
        return false;
    }
...
public:
    ...
    TlsHello() {
        ...
        static_assert(MAX_GREASE % 2 == 0, "MAX_GREASE must be even");
    ...
    struct Op {
        enum class Type {
            String, Random, K, M, P, E, Zero, Domain, Grease, BeginScope, EndScope, Permutation, DpiFinalPadding
        };
        Type type;
        size_t length;
        int seed;
        ...
        static Op dpi_final_padding() {
            Op res;
            res.type = Type::DpiFinalPadding;
            return res;
        }

    };

    // Returns a freshly built dpifix ClientHello on EVERY call (not a static
    // cache). This matters because grease[] is randomized per TlsHello instance
    // and Permutation reshuffles extensions each time, so a new object per
    // handshake yields a different-looking ClientHello on every proxy connect.
    static TlsHello getDpiFixDefault() {
        TlsHello res;
        // DPI-fix ClientHello (ported from tdesktop 6.8.4-dpifix by Qz3rK).
        // Updated Chrome-like fingerprint with fixed length, X25519 key
        // share only (no ML-KEM / ECH), and zero-padding to 517 bytes.
        res.ops = {
                    ...
                    Op::zero(83),
                    // tdesktop dpifix appends a final length-scoped zero block
                    // so the whole fake ClientHello is exactly 517 bytes even
                    // when the SNI domain length changes.
                    Op::dpi_final_padding()
            };
        return res;
    }


    static const TlsHello &getOriginalDefault() {
    ...
    // Returns the number of bytes written, or 0 if generation failed (buffer
    // overflow guard tripped or key generation failed). Callers MUST treat a
    // return value of 0 as an error and abort the handshake.
...
private:
    ...
    // Bounds check before writing `size` bytes at `offset`. Sets the error flag
    // and returns false if the write would exceed CLIENT_HELLO_LIMIT.
    bool grow(uint32_t offset, size_t size) {
        if (offset + size > CLIENT_HELLO_LIMIT) {
            error = true;
            return false;
        }
        return true;
    ...
    void writeOp(const TlsHello::Op &op, uint8_t *data, uint32_t &offset) {
        ...
        switch (op.type) {
            ...
            case Type::Random:
                ...
                break;
            case Type::K:
                // X25519 key share (32 bytes). Keep the original constant-time
                // Montgomery-curve generation; do NOT replace with Ed25519.
                if (!grow(offset, op.length) || !generate_public_key(data + offset)) {
                    error = true;
                    return;
                }
                offset += op.length;
                break;
            case Type::M:
                // ML-KEM-768 key share (1184 bytes), matching tdesktop's Chrome
                // post-quantum ClientHello fingerprint.
                if (!grow(offset, 1184)) {
                    return;
                }
            ...
            case Type::Grease: {
                if (op.seed < 0 || op.seed >= MAX_GREASE || !grow(offset, 2)) {
                    error = true;
                    return;
                }
            ...
            case Type::E: {
                const std::array<size_t, 4> lengths = {144, 176, 208, 240};
                const size_t length = lengths[tls_random_index(lengths.size())];
                if (!grow(offset, length)) {
                    return;
                }
                RAND_bytes(data + offset, length);
                offset += length;
                break;
            }
            case Type::P: {
            ...
            case Type::DpiFinalPadding: {
                static constexpr uint32_t kDpiFixClientHelloLength = 517;
                if (offset > kDpiFixClientHelloLength - 2) {
                    error = true;
                    return;
                }
                const uint32_t padding = kDpiFixClientHelloLength - 2 - offset;
                writeOp(Op::begin_scope(), data, offset);
                writeOp(Op::zero(padding), data, offset);
                writeOp(Op::end_scope(), data, offset);
                break;
            }
            case Type::Permutation: {
                std::vector<std::vector<Op>> list;
                list.reserve(op.entities.size());
                for (const auto &part : op.entities) {
                    list.push_back(part);
                }
                const size_t size = list.size();
                for (size_t i = 0; i + 1 < size; ++i) {
                    const size_t j = i + tls_random_index(size - i);
                    if (i != j) {
            ...
            if (proxyAuthState != 0) {
                if (proxyAuthState >= 10) {
                    if (proxyAuthState == 10) {
                        ...
                        proxyAuthState = 11;
                        TlsHello hello = useDpiFixTlsHello ? TlsHello::getDpiFixDefault() : TlsHello::getOriginalDefault();
                        hello.setDomain(currentSecretDomain);
                        uint32_t size = hello.writeToBuffer(tempBuffer->bytes);
                        if (size == 0) {
                            if (LOGS_ENABLED) DEBUG_E("connection(%p) could not generate TLS ClientHello", this);
                            closeSocket(1, -1);
                            return;
                        }
                        if (useDpiFixTlsHello) {
                            static constexpr uint32_t kDpiFixClientHelloLength = 517;
                            if (size != kDpiFixClientHelloLength) {
                                if (LOGS_ENABLED) DEBUG_E("connection(%p) DPI ClientHello invalid length: %u", this, size);
                                closeSocket(1, -1);
                                return;
                            }
                        }
                        ...
                        if (useDpiFixTlsHello) {
                            // DPI-fix for external MTProxy only: split the ClientHello
                            // into several non-blocking send() calls. Direct Telegram
                            // fakeTLS keeps the original one-shot send below.
                            uint32_t sendOffset = 0;
                            int eagainRetries = 0;
                            bool sendFailed = false;
                            while (sendOffset < size) {
                                uint32_t remainingHello = size - sendOffset;
                                uint32_t chunk = 48 + (tls_random_u32() % 49); // 48..96
                                if (chunk > remainingHello) {
                                    chunk = remainingHello;
                                }
                                ssize_t sent = send(socketFd, tempBuffer->bytes + sendOffset, chunk, 0);
                                if (sent > 0) {
                                    sendOffset += (uint32_t) sent;
                                } else if (sent < 0 && (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR)) {
                                    // Non-blocking socket is temporarily not ready. We do
                                    // not have a full async split state machine here, so
                                    // retry only a bounded number of times to avoid a busy
                                    // loop inside epoll callback.
                                    if (++eagainRetries > 32) {
                                        sendFailed = true;
                                        break;
                                    }
                            ...
                            if (sendFailed) {
                                if (LOGS_ENABLED) DEBUG_E("connection(%p) DPI ClientHello chunked send failed", this);
                                closeSocket(1, -1);
                                return;
                            }
                        } else {
